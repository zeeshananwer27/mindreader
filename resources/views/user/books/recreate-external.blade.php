                        ...
                        <div class="card-body">
                            <div id="step1">
                                <div class="form-group mb-4">
                                    <label for="file">{{ translate("Upload Book") }}</label>
                                    <input name="file" class="flex w-full px-3 py-1" accept=".pdf" id="file"
                                           type="file">
                                    <small
                                        class="text-muted">{{ translate("Upload a PDF book, and our magical AI will do the rest.") }}</small>
                                </div>
                                <div class="row">
    ...
    <script nonce="{{ csp_nonce() }}">
        $(document).ready(function () {
            $('#generate_synopsis').on('click', function () {
                // Get the file from the input field by ID
                ...
                formData.append('author_profile_id', $('#authorProfile').val());
                formData.append('language', $('#language').val());
                formData.append('is_personal', $('#isPersonal').val());
                formData.append('purpose', $('#change').val());
                formData.append('target_audience', $('#targetAudience').val());
            ...
            $('#book_outline').on('click', function () {
                ...
                // Perform AJAX request
                $.ajax({
                    url: '{{route('user.book.manager.recreate.external.store')}}',
                    type: 'POST',
                    data: formData,
                    success: function (response) {
                        hideLoadingSwal();
